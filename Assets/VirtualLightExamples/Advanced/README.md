...
2. `Scenes/VirtualLightFeatureLab.unity`
   ...
   - metallic/smoothnessの違い、Rectangle Spotの四角いdirect light / custom shadow、円形のbeam / impact、最初のhitで止まるhard-stop beamを確認します。
...
4. `Scenes/VirtualLightArenaSample.unity`
   ...
   - Fan、Cross、Converge、Soloの4フェーズを24秒で切り替え、6本すべてのbeamを最初の分類済みColliderで止めます。

Package側のCore Feature Matrixは、6ステーションを静的に比較する導入・Shape確認用の1シーンです。Spotのshadow / beam / impact、Rectangle Areaの方向性、moving head演出、負荷測定は役割を混在させず、このAdvancedフォルダーと`Assets/VirtualLightExamples/PerformanceBenchmark`で確認してください。
