...
5. Optionally assign a unit Cube using `MizoTake/Virtual Light/Beam` to Beam Visual and an impact Transform to Impact Visual.
6. Assign a grayscale `Gobo / Cookie` Texture2D on the Spot to mask its opaque contribution, beam cross-section, and impact footprint with the same pattern.
...
For a low-cost hard-stop beam, enable `Truncate Visual At First Hit`, keep `Probe Radius` at zero when a center Raycast is sufficient, restrict `Occluder Layers`, keep `Require Occluder Marker` enabled, and cap automatic refresh at 30-60 Hz. Truncation reuses the existing hit and shrinks the rendered proxy instead of adding a cutoff branch to the beam shader. This reduces the pixels covered when the beam is blocked, while the selected 12/20/32 raymarch steps remain unchanged for each surviving pixel. Move dynamic Collider blockers through the Physics timeline, such as a kinematic Rigidbody in `FixedUpdate`, instead of calling the global `Physics.SyncTransforms()` independently for every beam.
...
The custom shadow path targets registered opaque Renderers for every Virtual Light type; the beam sharing described here remains Spot-specific. Alpha-clipped and transparent casting are not guaranteed, Rectangle Area uses a center-projection approximation, Directional is non-cascaded, and multiple scattering or physically complete volumetric penumbra remain outside the current scope. The shadow texture and GPU metadata grow with demand instead of enforcing a package-authored count, but platform resource limits and frame-time remain practical constraints.
