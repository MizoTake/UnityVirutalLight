...
- Added custom shadows for Point (six faces), Spot, Rectangle Area (front/back center approximation), and Directional (camera-centered non-cascaded orthographic), plus 2D Gobo/Cookie masks shared by opaque lighting and Spot beam/impact visuals.
- Replaced per-tile light-index lists with 32-lights-per-uint masks generated by one 64-thread compute group per tile; pixel shaders enumerate set bits with `firstbitlow`, and the layout is ready for a future Z Bin mask AND.
...
- Added Directional Virtual Lights with position- and range-independent direct lighting, full-screen tile selection, forward-direction editor gizmos, and Unity Directional Light conversion; the current Unreleased implementation now adds camera-centered non-cascaded Directional shadows.
