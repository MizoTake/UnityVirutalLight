...
本仕様では、リアルタイム性と拡張性のバランスから、**StructuredBufferによる仮想ライト管理、Compute Shaderによるライト選別・光場更新、マテリアルシェーダーによる最終評価**を組み合わせたハイブリッド構成を採用する。
...
8. ライト数が上限を超えた場合、優先度に基づいて安全に削減できること。
...
- ライト数やクラスタ容量の超過時も描画破綻させない。
- 品質設定によってライト数、影、光場解像度を変更できる。
- 非対応環境では、少数ライトの直接評価へフォールバックできる。
...
5. **遮蔽**  
   優先ライトにはシャドウマップまたはレイトレーシング、その他にはSDF・ボクセル・スクリーンスペース近似を使用する。
...
| フラグメントシェーダーで全ライト評価 | 実装が単純 | ピクセル数 × ライト数で負荷増加 | 最大8灯程度のフォールバック |
...
1ライトあたり80 byteとする。256ライト時のライト本体データは20 KiBである。
...
- 不正値、未対応Scale、ライト上限超過、影スロット不足をHelpBoxで通知する。
...
- 1クラスタの上限を超えたライトは、推定寄与の低いものから除外する。
- 推定寄与は、強度、距離減衰、投影面積、優先度を用いて算出する。
- デバッグビルドでは超過クラスタを色表示する。
...
| Shadow Atlas | 高 | 中〜高 | 優先ライト |
| Ray Traced Shadow | 高 | 高 | 対応環境の高品質設定 |

### 13.2 推奨デフォルト

- 優先度上位4灯: Shadow Atlas
- その他: SDFまたはVoxel近似
- 生成VPL: 原則として近似遮蔽
- 高品質設定: 影上位ライトをレイトレーシングへ置換可能
...
| 設定 | 最大ライト数 | 1クラスタ上限 | 影 | 光場 | VPL |
|---|---:|---:|---|---|---|
| Low | 16 | 8 | なし | なし | なし |
| Medium | 64 | 16 | 上位2灯 | 2Dのみ | 最大8 |
| High | 128 | 32 | 上位4灯 + 近似遮蔽 | 2D / 3D | 最大32 |
| Ultra | 256 | 64 | RTまたは高品質Atlas | 高解像度3D | 最大64 |

プリセット値は上限であり、プラットフォーム別に上書き可能とする。
...
| Shadow Atlas 2048 depth | フォーマット依存 |

---

## 18. フォールバック仕様

Compute Shaderまたは必要機能が使用できない場合、以下へ切り替える。

1. 最大8灯までに制限する。
2. CPUでカメラへの寄与が高いライトを選択する。
3. マテリアルシェーダーで固定長ループ評価する。
4. 光場、VPL自動生成、体積注入を無効化する。
5. 影は無効、または標準ライトへ変換した上位1灯のみとする。
...
- GPUバッファ確保失敗時はフォールバックへ移行する。
- ライト上限超過時は例外ではなく優先度削減を行う。
...
| リスク | 症状 | 対策 |
|---|---|---|
| 光漏れ | 壁裏や閉空間が明るくなる | Shadow Atlas、SDF、Voxel、厚み推定 |
| ライト数増大 | GPU時間が急増 | Clustered Lighting、上限、優先度削減 |
...
StructuredBufferで仮想ライト管理
    ↓
Tiled / Clustered Lightingで対象ライトを限定
    ↓
既存PBRへ追加光として反映
    ↓
上位ライトのみ影を付与
